...
class MoCoRelEncoderTextModalAux(BaseNetwork):
    def __init__(
        self, config, device, n_rel_cls, out_dim, 
        t_emb_vec: torch.Tensor, none_emb: torch.Tensor, 
        multi_gpu=False
    ):
        super(MoCoRelEncoderTextModalAux, self).__init__()
        self.config = config
    ...
    def _train(self, pcd_q, pcd_k, labels):
        ...
        return features, target, torch.cat([logits_q, logits_n], dim=1), aux_desc # B X (C + 1)
    ...
    def _inference(self, pcd):
        ...
        aux_desc = self.linear_aux(q)
        return q_G, torch.cat([logits_q, logits_n], dim=1), aux_desc
