...
class MoCoRelEncoderBaseline(BaseNetwork):
    def __init__(self, config, device, n_rel_cls, out_dim, multi_gpu=False):
        super(MoCoRelEncoderBaseline, self).__init__()
        self.config = config
        ...
        self.centers = nn.Parameter(torch.randn(n_rel_cls, self.edge_dim))
        self.none_centers = nn.Parameter(torch.randn(1, self.edge_dim))
        with torch.no_grad():
            self.centers.data = F.normalize(self.centers.data, dim=1)
            self.none_centers.data = F.normalize(self.none_centers.data, dim=1)
