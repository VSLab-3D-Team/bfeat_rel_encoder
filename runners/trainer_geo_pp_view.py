...
class BFeatRelGeoViewPPTrainer(BaseTrainer):
    ...
    def train(self):
        ...
        for e in range(1, self.t_config.epoch + 1):
            ...
            for idx, (
                ...
                _, pred_desc, pred_sub_aabb, pred_obj_aabb, pred_viewpoint = \
                    self.model(rel_pts)
                ...
                t_loss = 0.1 * desc_loss + 0.1 * (s_bbox_loss + o_bbox_loss) + view_loss
                t_loss.backward()
                self.optimizer.step()
    ...
    def evaluate_validation(self):
        ...
        with torch.no_grad():
            ...
            for _, (
                ...
                rel_pts = rel_pts.float().contiguous().transpose(2, 1)
                gt_view_feat = gt_view_feat.float()
                features, pred_desc, pred_sub_aabb, pred_obj_aabb, pred_viewpoint = \
                    self.model(rel_pts)
                ...
                view_loss = F.l1_loss(pred_viewpoint, gt_view_feat) # self.cosine_loss
                v_loss = 0.1 * v_desc_loss + 0.1 * (v_s_bbox_loss + v_o_bbox_loss) + view_loss
