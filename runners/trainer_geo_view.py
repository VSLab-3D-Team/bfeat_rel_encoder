...
class BFeatRelGeoViewTrainer(BaseTrainer):
    def __init__(self, config, device, multi_gpu=False):
        super().__init__(config, device, multi_gpu, is_edge_view=True)
        
        self.m_config = config.model
        # Model Definitions
        self.model = GeoViewRelEncoder(
    ...
    def train(self):
        ...
        for e in range(1, self.t_config.epoch + 1):
            ...
            for idx, (
                ...
                rel_pts, gt_descriptor, gt_sub_aabbox, gt_obj_aabbox, gt_view_feat = \
                    self.to_device(rel_pts, gt_descriptor.float(), gt_sub_aabbox.float(), gt_obj_aabbox.float(), gt_view_feat)
                self.optimizer.zero_grad()
                rel_pts = rel_pts.float().contiguous().transpose(2, 1)
                gt_view_feat = gt_view_feat.float()
                _, trans, pred_desc, pred_sub_aabb, pred_obj_aabb, pred_viewpoint = \
                    self.model(rel_pts)
                ...
                view_loss = F.l1_loss(pred_viewpoint, gt_view_feat) # self.cosine_loss
                t_loss = 0.5 * desc_loss + 0.1 * (s_bbox_loss + o_bbox_loss) + view_loss
                t_loss.backward()
                self.optimizer.step()
                ...
                self.meters['Train/View_Reg_Loss'].update(view_loss.detach().item())
                t_log = [
                    ...
                    ("train/v_feat_reg", view_loss.detach().item()),
                    ("Misc/epo", int(e)),
                    ("Misc/it", int(idx)),
    ...
    def evaluate_validation(self):
        ...
        with torch.no_grad():
            ...
            for _, (
                ...
                rel_pts, gt_rel_label, gt_descriptor, gt_sub_aabbox, gt_obj_aabbox, gt_view_feat = \
                    self.to_device(rel_pts, gt_rel_label, gt_descriptor, gt_sub_aabbox, gt_obj_aabbox, gt_view_feat)
                self.optimizer.zero_grad()
                rel_pts = rel_pts.float().contiguous().transpose(2, 1)
                gt_view_feat = gt_view_feat.float()
                features, trans, pred_desc, pred_sub_aabb, pred_obj_aabb, pred_viewpoint = \
                    self.model(rel_pts)
                ...
                view_loss = F.l1_loss(pred_viewpoint, gt_view_feat) # self.cosine_loss
                v_loss = 0.5 * v_desc_loss + 0.1 * (v_s_bbox_loss + v_o_bbox_loss) + view_loss
                ...
                self.meters['Validation/View_Reg_Loss'].update(view_loss.detach().item())
                ...
                t_log = [
                    ...
                    ("val/v_feat_reg", view_loss.detach().item()),
                ]
                progbar.add(1, values=t_log)
