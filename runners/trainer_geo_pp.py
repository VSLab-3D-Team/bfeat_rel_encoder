...
class BFeatRelGeoPPTrainer(BaseTrainer):
    ...
    def train(self):
        ...
        for e in range(1, self.t_config.epoch + 1):
            ...
            for idx, (
                ...
                _, pred_desc, pred_sub_aabb, pred_obj_aabb = \
                    self.model(rel_pts)
                ...
                t_loss = desc_loss + 0.5 * (s_bbox_loss + o_bbox_loss) # 0.1 *
                t_loss.backward()
                self.optimizer.step()
    ...
    def evaluate_validation(self):
        ...
        with torch.no_grad():
            ...
            for _, (
                ...
                self.optimizer.zero_grad()
                rel_pts = rel_pts.float().contiguous().transpose(2, 1)
                features, pred_desc, pred_sub_aabb, pred_obj_aabb = \
                    self.model(rel_pts)
                ...
                v_loss = v_desc_loss + 0.5 * (v_s_bbox_loss + v_o_bbox_loss)
