...
class SSGRelGeoViewFeatEncoder3D(Dataset):
    def __init__(
        self, config, split, device, 
        scan_data, relationship_json, objs_json, scans, 
        o_obj_cls, o_rel_cls
    ):
        super(SSGRelGeoViewFeatEncoder3D, self).__init__()
        ...
        for scan_data in tqdm(self.scan_data, total=len(self.scan_data)):
            rel_pts, gt_rels, rel_desc, sub_aabb, obj_aabb, rel_clip_feat_mean = \
                self.__get_data(
                    scan_data["points"], scan_data["mask"], self.config.num_points_reg, self.relationNames, 
                    scan_data["map_instid_name"], scan_data["rel_json"], scan_data["edge_view_rgb"],
            ...
            self.rel_feats.append(rel_clip_feat_mean)
        ...
        self.rel_feats = torch.cat(self.rel_feats, dim=0)
    ...
    def __get_data(
        ...
        rel_clip_feat_mean = torch.from_numpy(rel_viewpoint_clip)
        return rel_points[..., :self.dim_pts], gt_rels, rel_descriptor, sub_aabb, obj_aabb, rel_clip_feat_mean
