...
def __read_3dssg_scans_viewpoint(args):
    ...
    for i, rel in enumerate(relaiontships):
        ...
        rel_viewpoint.append(edge_view_np)
...
def read_scan_data(config, split, device):
    ...
    _, _, o_obj_cls, o_rel_cls = compute(classNames, relationNames, data, selected_scans, False)
    ...
    return scan_data, relationship_json, objs_json, scans, o_obj_cls, o_rel_cls
